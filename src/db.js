...
export async function obtenerDashboard(userId) {
  const [{ data: examenes }, { data: racha }, { data: errores }] = await Promise.all([
    supabase.from("examenes").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(10),
    supabase.from("rachas").select("*").eq("user_id", userId).single(),
    supabase.from("respuestas_detalle").select("categoria, es_correcta").eq("user_id", userId),
  ]);
